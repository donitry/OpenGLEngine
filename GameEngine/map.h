...
bool MAP::InsertVertex(long obj, GLdouble x, GLdouble y, GLdouble z, GLfloat r, GLfloat g, GLfloat b, GLfloat a, GLfloat nx, GLfloat ny, GLfloat nz, GLfloat fogdepth)
{
	...
	else
	{
		...
		for (long i = 0; i < object[obj].max_vertices; i++) object[obj].vertex[i] = temp[i];
		delete [] temp;
		temp = NULL;
	}
...
bool MAP::InsertTriangle(long obj, GLint p1, GLint p2, GLint p3, GLfloat u1, GLfloat v1, GLfloat u2, GLfloat v2, GLfloat u3, GLfloat v3)
{
	...
	else 
	{
		...
		for (long i = 0; i < object[obj].max_triangles; i++) object[obj].triangle[i] = temp[i];
...
bool MAP::InsertEntity(long type, double x, double y, double z, float xa, float ya, float za, long health, long strength, long armour)
{
	...
	else
	{
		...
		for (long i = 0; i < header.max_entities; i++) entity[i] = temp[i];
		delete [] temp;
	}
...
bool MAP::InsertItem(GLdouble x, GLdouble y, GLdouble z, GLint type, GLint respawn_wait, GLint respawn_time)
{
	...
	else
	{
		...
		for (long i = 0; i < header.max_items; i++) item[ i ] = temp[ i ];
...
bool MAP::InsertSound(GLdouble x, GLdouble y, GLdouble z, char *filename, GLfloat xa, GLfloat ya, GLfloat za)
{
	...
	else
	{
		...
		for (long i = 0; i < header.max_sounds; i++) sound[ i ] = temp[ i ];
...
bool MAP::InsertTexture(long obj, char *filename, long style)
{
	...
	else 
	{
		...
		for (long i = 0; i < object[ obj ].max_textures; i++) object[ obj ].texture[ i ] = temp[ i ];
		delete [] temp;
	}
...
void MAP::Save(char *filename)
{
	...
	fp = fopen (filename, "wb");
		...
		for (long i = 0; i < header.max_objects; i++)
		{
			...
			for (long i2 = 0; i2 < object[i].max_vertices; i2++) fwrite (&object[i].vertex[i2], sizeof(MAP_VERTEX), 1, fp);
			for (long i2 = 0; i2 < object[i].max_triangles; i2++) fwrite (&object[i].triangle[i2], sizeof(MAP_TRIANGLE), 1, fp);
			for (long i2 = 0; i2 < object[i].max_textures; i2++) fwrite (&object[i].texture[i2], sizeof(MAP_TEXTURE), 1, fp);
		}

		for (long i = 0; i < header.max_entities; i++) fwrite (&entity[i], sizeof(MAP_ENTITY), 1, fp);		
		for (long i = 0; i < header.max_lights; i++)
		{
			fwrite (&light[i], sizeof(MAP_LIGHT), 1, fp);
			for (long i2 = 0; i2 < light[i].max_inclusions; i2++) fwrite (&light[i].inclusions[i2], sizeof(long), 1, fp);
		}

		for (long i = 0; i < header.max_sounds; i++) fwrite (&sound[i], sizeof(MAP_SOUND), 1, fp);			
		for (long i = 0; i < header.max_items; i++) fwrite (&item[i], sizeof(MAP_ITEM), 1, fp);
	...
	if (fp == NULL) return (false);
		...
		if (header.max_objects > 0)
		{
			...
			for (long i = 0; i < header.max_objects; i++)
			{
				...
				for (long i2 = 0; i2 < object[i].max_vertices; i2++) fread (&object[i].vertex[i2], sizeof(MAP_VERTEX), 1, fp);
				for (long i2 = 0; i2 < object[i].max_triangles; i2++) fread (&object[i].triangle[i2], sizeof(MAP_TRIANGLE), 1, fp);
				for (long i2 = 0; i2 < object[i].max_textures; i2++) fread (&object[i].texture[i2], sizeof(MAP_TEXTURE), 1, fp);
			}
		}
		else object = NULL;
